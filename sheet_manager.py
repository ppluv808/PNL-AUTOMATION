...
def append_or_update_summary(sheet, today, data):
    """
    Update the sheet for the specific date.
    """
    ...
    payin_sum = sum(float(t.get("amount", 0)) for t in payins)
    payout_sum = sum(float(t.get("amount", 0)) for t in payouts)
    ...
    gcash_fee = sum(float(t.get("fee", 0)) for t in gcash_txn)
    qrph_fee = sum(float(t.get("fee", 0)) for t in qrph_txn)
    payout_fee = sum(float(t.get("fee", 0)) for t in payout_txn)

    total_fees = gcash_fee + qrph_fee + payout_fee
    buy_rate = round(total_fees / payin_sum, 6) if payin_sum else 0

    # Ensure we're comparing string dates in mm/dd/yyyy format
    date_str = today.strftime("%m/%d/%Y")
    sheet_data = sheet.get_all_records()
    row_idx = None

    for idx, row in enumerate(sheet_data, start=2):  # Skip header row
    ...
    # Update B row onwards (columns B to K)
    sheet.update(f"B{row_idx}", [values])

    # Read Settlement value (L) as user input
    try:
        settlement_val = sheet.cell(row_idx, 12).value
        settlement = float(settlement_val) if settlement_val else 0.0
    except:
        settlement = 0.0
    ...
    # Update Ending Balance in column M (13)
